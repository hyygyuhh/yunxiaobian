...
// 扫码登录相关元素
const tabCookieLogin = document.getElementById('tabCookieLogin')
const tabQrLogin = document.getElementById('tabQrLogin')
const panelCookieLogin = document.getElementById('panelCookieLogin')
const panelQrLogin = document.getElementById('panelQrLogin')
const appQrCodeImg = document.getElementById('appQrCodeImg')
const appQrCodeWrap = document.getElementById('appQrCodeWrap')
const appQrOverlay = document.getElementById('appQrOverlay')
const appQrOverlayText = document.getElementById('appQrOverlayText')
const appQrRefreshBtn = document.getElementById('appQrRefreshBtn')
const appQrStatus = document.getElementById('appQrStatus')
const appQrSuccess = document.getElementById('appQrSuccess')
const appQrNickname = document.getElementById('appQrNickname')
const appQrStartBtn = document.getElementById('appQrStartBtn')
...
let appQrPollTimer = null
let appQrKey = ''

function stopAppQrPoll() {
  if (appQrPollTimer) {
    clearInterval(appQrPollTimer)
    appQrPollTimer = null
  }
}

function switchLoginTab(tab) {
  tabCookieLogin.classList.toggle('active', tab === 'cookie')
  tabQrLogin.classList.toggle('active', tab === 'qr')
  panelCookieLogin.classList.toggle('hidden', tab !== 'cookie')
  panelQrLogin.classList.toggle('hidden', tab !== 'qr')
  if (tab === 'qr') {
    appQrStartBtn.textContent = appQrKey ? '刷新二维码' : '获取二维码'
  }
}

async function startAppQrLogin() {
  appQrSuccess.classList.add('hidden')
  appQrOverlay.classList.add('hidden')
  appQrCodeImg.style.display = 'block'
  appQrStatus.textContent = '正在获取二维码…'
  appQrStartBtn.disabled = true

  try {
    const keyRes = await chrome.runtime.sendMessage({ type: 'QR_GET_KEY' })
    if (!keyRes?.key) {
      appQrStatus.textContent = keyRes?.error || '获取二维码失败，请重试'
      return
    }
    appQrKey = keyRes.key

    const imgRes = await chrome.runtime.sendMessage({ type: 'QR_GET_IMAGE', key: appQrKey })
    if (imgRes?.ok && imgRes.result) {
      appQrCodeImg.src = imgRes.result.img
      appQrStatus.textContent = '请使用网易云音乐 APP 扫描二维码'
      appQrStartBtn.textContent = '刷新二维码'
    } else {
      appQrStatus.textContent = '生成二维码图片失败'
      return
    }

    stopAppQrPoll()
    appQrPollTimer = setInterval(() => pollAppQrStatus(), 3000)
  } catch (err) {
    appQrStatus.textContent = '获取二维码失败：' + (err?.message || err)
  } finally {
    appQrStartBtn.disabled = false
  }
}

async function pollAppQrStatus() {
  if (!appQrKey) return

  try {
    const res = await chrome.runtime.sendMessage({ type: 'QR_CHECK_STATUS', key: appQrKey })
    if (!res) return

    if (res.loggedIn) {
      stopAppQrPoll()
      appQrCodeImg.style.display = 'none'
      appQrOverlay.classList.add('hidden')
      appQrSuccess.classList.remove('hidden')
      appQrNickname.textContent = res.nickname ? `欢迎，${res.nickname}` : '登录成功'
      appQrStatus.textContent = '登录成功，Cookie 已写入'
      appQrStartBtn.textContent = '获取二维码'
      appQrKey = ''

      // 验证登录并更新 UI
      setTimeout(async () => {
        try {
          const cookieRes = await chrome.runtime.sendMessage({ type: 'GET_COOKIE' })
          if (cookieRes?.ok && cookieRes.cookieString) {
            setCookie(cookieRes.cookieString)
            await refreshLoginUI()
            closeCookieModal()
            showSuccess(`登录成功，欢迎 ${res.nickname || ''}`)
          }
        } catch {}
      }, 600)
      return
    ...
    switch (res.code) {
      case 800:
        stopAppQrPoll()
        appQrOverlay.classList.remove('hidden')
        appQrOverlayText.textContent = '二维码已过期'
        appQrStatus.textContent = '二维码已过期，请点击刷新'
        break
      case 802:
        appQrStatus.textContent = '已扫码，请在手机上确认登录'
        break
      case 801:
        break
      default:
        if (res.message) appQrStatus.textContent = res.message
    }
  } catch {
    // 网络错误不中断轮询
  }
...
// 扫码登录 Tab 切换与操作
tabCookieLogin?.addEventListener('click', () => switchLoginTab('cookie'))
tabQrLogin?.addEventListener('click', () => switchLoginTab('qr'))
appQrStartBtn?.addEventListener('click', startAppQrLogin)
appQrRefreshBtn?.addEventListener('click', startAppQrLogin)
